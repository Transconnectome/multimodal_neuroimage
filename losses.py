...
class Cont_Loss(nn.Module):
    ...
    def forward(self, input):
        ...
        return self.cont_loss
'''
## Stella added this for lowfreqBERT
class Merge_Loss(nn.Module):
    def __init__(self,**kwargs):
        super(Merge_Loss, self).__init__()
        #task = kwargs.get('task')
        self.merge_loss = None
    def cosine_similarity(self, A, B):
        cos = nn.CosineSimilarity(dim=0)
        return cos(A, B)
    def squared_distance(self, A, B):
        return torch.sum(torch.square((A-B)))
    def forward(self, processed_raw, embedding_per_ROIs):
        batch, roi = processed_raw.shape
        margin = 1
        loss = 0
        # 같은 subject의 merged_divided (process한 low, ultralow 합친 것) 과 processed_raw (process한 raw)이 비슷하도록!
        for i in range(batch):
            for j in range(batch):
                ## cosine similarity
                cos_sim = self.cosine_similarity(embedding_per_ROIs[i, :], processed_raw[j, :])
                ...
                if i == j:
                    ...
                    label = 0    
                loss_function = label*cos_sim + (1 - label)*(max (0, (margin - cos_sim)))
                loss+=loss_function
        self.merge_loss = loss/(batch**2)
        #print('merge loss is:', self.merge_loss)
        return self.merge_loss
