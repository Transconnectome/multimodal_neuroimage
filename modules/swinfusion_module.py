...
def window_reverse_fusion(windows, window_size, H, W):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
class WindowAttention_fusion(nn.Module):
...
class SwinTransformerBlock_fusion(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention_fusion(
        ...
        self.mlp = Mlp_fusion(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def calculate_mask(self, x_size):
        ...
        mask_windows = window_partition_fusion(img_mask, self.window_size)  # nW, window_size, window_size, 1
    ...
    def forward(self, x, x_size):
        ...
        # partition windows
        x_windows = window_partition_fusion(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
        ...
        shifted_x = window_reverse_fusion(attn_windows, self.window_size, H, W)  # B H' W' C
...
class Cross_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp_A = Mlp_fusion(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp_B = Mlp_fusion(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, y, x_size):
        ...
        x_windows = window_partition_fusion(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C
        y_windows = window_partition_fusion(shifted_y, self.window_size)  # nW*B, window_size, window_size, C
        ...
        shifted_x = window_reverse_fusion(attn_windows_A, self.window_size, H, W)  # B H' W' C
        shifted_y = window_reverse_fusion(attn_windows_B, self.window_size, H, W)  # B H' W' C
...
class PatchMerging_fusion(nn.Module):
...
class BasicLayer_fusion(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock_fusion(dim=dim, input_resolution=input_resolution,
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = BasicLayer_fusion(dim=dim,
...
class CRSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = Cross_BasicLayer(dim=dim,
                                         ...
                                         downsample=downsample,
                                         use_checkpoint=use_checkpoint)
        self.residual_group_A = BasicLayer_fusion(dim=dim,
                                         input_resolution=input_resolution,
                                         depth=depth,
                                         num_heads=num_heads,
                                         window_size=window_size,
                                         ...
                                         downsample=downsample,
                                         use_checkpoint=use_checkpoint)
        self.residual_group_B = BasicLayer_fusion(dim=dim,
                                         input_resolution=input_resolution,
                                         depth=depth,
                                         num_heads=num_heads,
                                         window_size=window_size,
...
class PatchEmbed_fusion(nn.Module):
