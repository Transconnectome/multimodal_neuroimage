...
def _get_sync_file():    
        ...
        sync_file_dir = '/scratch/connectome/stellasybae/lowfreqBERT/pytorch-sync-files' # Stella lab server
        '''
        if 'SCRATCH' in os.environ:
            sync_file_dir = '%s/pytorch-sync-files' % os.environ['/home/scratch/connectome/stellasybae/lowfreqBERT'] # Stella lab server
        else:
...
def init_distributed(args):   
    ...
    print('args.non_distributed is:', args.non_distributed)
...
def weight_loader(args):
    ...
    try:
        if args.step == '1' :
            ...
            task = 'FuncStruct'
            if os.path.exists(args.model_weights_path_phase3):
                model_weights_path = args.model_weights_path_phase3 # for transfer learning
        elif args.step == '6':
            task = 'SwinFusion'
