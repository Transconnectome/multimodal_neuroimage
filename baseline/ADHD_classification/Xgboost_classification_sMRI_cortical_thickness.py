...
raw_data_smri = raw_data_smri.drop_duplicates(subset='subjectkey', keep='first',inplace=False)
...
manufactured_data = pd.merge(valid_sub, raw_data_smri[new_thickness_collist], how='inner', on='subjectkey')

phen = pd.read_csv('../data/metadata/ABCD_phenotype_total.csv')

data = pd.merge(phen[['subjectkey', 'ADHD_label']], manufactured_data, how='inner', on='subjectkey')
...
start_brain_index = np.where(data.columns.values == "lh_bankssts_thickness._.5")[0][0]
