...
raw_data_fmri = raw_data_fmri.drop_duplicates(subset='subjectkey', keep='first',inplace=False)
...
manufactured_data = pd.merge(valid_sub, raw_data_fmri, how='inner', on='subjectkey')
...
start_brain_index = np.where(data.columns.values == "1")[0][0]
