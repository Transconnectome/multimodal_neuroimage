...
# 01 import data
raw_data_dti = pd.read_csv('../data/count.qc.csv') # DTI - count
raw_data_smri = pd.read_csv('../data/mor_final.csv')
thickness_collist = []
for col in raw_data_smri.columns:
    if 'thickness' in col:
        thickness_collist.append(col)
        
new_thickness_collist = []
for col in thickness_collist:
    if '._.5' in col:
        new_thickness_collist.append(col)
    elif '._.6' in col:
        new_thickness_collist.append(col)
        
new_thickness_collist.append('subjectkey')
new_thickness_collist.remove('lh_MeanThickness_thickness._.5')
new_thickness_collist.remove('rh_MeanThickness_thickness._.6')

raw_data_fmri = pd.read_csv('../data/ABCD_rsfMRI_flatten_FC.csv') # fMRI flattened FC

raw_data_dti['subjectkey'] = raw_data_dti['subjectkey'].apply(lambda x : x.replace('_', ''))
raw_data_smri['subjectkey'] = raw_data_smri['subjectkey'].apply(lambda x : x.replace('_', ''))
raw_data_fmri['subjectkey'] = raw_data_fmri['subjectkey'].apply(lambda x : x.replace('_', ''))


phen = pd.read_csv('../data/metadata/ABCD_phenotype_total.csv')

data1 = pd.merge(phen[['subjectkey', 'ADHD_label']], raw_data_dti, how='inner', on='subjectkey')
data2 = pd.merge(raw_data_smri[new_thickness_collist], raw_data_fmri, how='inner', on='subjectkey')
data = pd.merge(data1, data2, how='inner', on='subjectkey')
...
data = pd.merge(valid_sub, data, how='inner', on='subjectkey')
data = data.dropna()
data = data.drop_duplicates(subset='subjectkey', keep='first',inplace=False)
print(len(data))

start_subjectkey_index = np.where(data.columns.values == "subjectkey")[0][0]
start_meta_index = np.where(data.columns.values == "ADHD_label")[0][0] # Gender for hcp, sex for ABCD
...
data = data[subjectkey+['ADHD_label']+brain]
data=data.dropna()
